...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in xcl.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("xcl.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* XCL features */
    { "xcl",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "xcl",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "xcl",               "gobject",                &gobject,                true,  {} },
    { "xcl",               "voteraw",                &voteraw,                true,  {} },
