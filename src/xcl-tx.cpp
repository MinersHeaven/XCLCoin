...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("XCL Core xcl-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  xcl-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded xcl transaction") + "\n" +
              "  xcl-tx [options] -create [commands]   " + _("Create hex-encoded xcl transaction") + "\n" +
              "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded xcl transaction
